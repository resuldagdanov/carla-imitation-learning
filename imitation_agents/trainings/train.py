...
def calculate_action_loss(throttle, steer, gt, criterion_throttle, criterion_steer):
...
def trainer(writer_counter):
    ...
    for data in tqdm(train_dataset_loader):
        ...
        speed_input = data['velocity'].to(device)
        control_input = data['control'].to(device)
        target_point = torch.stack(data['target_point'], dim=1).to(device)

        # concatenate ego vehicle velocity with local relative waypoints
